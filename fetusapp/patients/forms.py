...
from wtforms import StringField, RadioField, SubmitField, BooleanField, IntegerField, FloatField, TextAreaField, DateField
...
class HistoryMedicalForm(FlaskForm):
    ...
    weight = FloatField('Βάρος(kg):', validators=[Optional(), NumberRange(min=30, max=200)])
    height = FloatField('Ύψος(m):', validators=[Optional(), NumberRange(min=100, max=220)])
    ...
    transfusions_yn = RadioField('Μεταγγίσεις:', choices=[(True, 'Yes'), (False, 'No')], coerce=lambda x: x == 'True')
    transfusions_reactions = TextAreaField('Αντιδράσεις σε μεταγγίσεις:', validators=[Optional()])
    ...
    gynecological_surg = StringField('Γυναικολογικές επεμβάσεις:', validators=[Optional()])
